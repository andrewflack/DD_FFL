...
playoff_seeds <- results %>%
  ...
  arrange(year, desc(wins), desc(pts_for)) %>%
  mutate(division_winner = ifelse(seq_along(pts_for) == 1, TRUE, FALSE)) %>%
  ungroup() %>%
  arrange(year, desc(division_winner), desc(wins), desc(pts_for)) %>%
  group_by(year) %>% 
  mutate(seed = seq_along(pts_for)) %>% 
  select(year, owner, seed)
...
playoff_matchups_15 <- results_14 %>%
  ...
  mutate(bracket_section_15 = ifelse(won_matchup, paste0("winners_", bracket_section), "losers_bracket")) %>%
  ungroup() %>%
  group_by(year, bracket_section_15) %>%
  mutate(bracket_section_15_order = seq_along(bracket_section_15) <= 2) %>% 
  group_by(year, bracket_section_15, bracket_section_15_order) %>% 
