...
elohist_n <- array(NA, dim = c(length(sort(unique(c(results_w_elo$owner, results_w_elo$opp_owner)))), length(unique(results$year))*weeks), dimnames = list(sort(unique(results_w_elo$owner)), seq(1:(length(unique(results$year))*weeks))))
...
for (i in 1:nrow(results_w_elo)) {
  ...
  # if (results_w_elo$week[i] == 14) {
  #   results_w_elo$elo_i[i] <- init*revert_playoffs + results_w_elo$elo_i[i]*(1-revert_playoffs)
  # }
  ...
  print(paste0("Owner elo_i: ", round(results_w_elo$elo_i[i]), " opp_owner elo_i: ", round(elohist_i[results_w_elo$opp_owner[i], results_w_elo$period[i]]), " elo_diff: ", round(results_w_elo$elo_diff[i])))
